...
public class FoodItemCell : MonoBehaviour, IUICell<FoodItem>, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        // Find targets under cursor
        var results = new List<RaycastResult>();
        EventSystem.current.RaycastAll(eventData, results);

        // Check for NpcBehaviorLogic
        NpcBehaviorLogic targetNpc = null;
        FoodItemCell targetCell = null;

        foreach (var result in results)
        {
            // First check for NPC
            targetNpc = result.gameObject.GetComponent<NpcBehaviorLogic>();
            if (targetNpc != null)
                break;

            // Then check for another FoodItemCell
            targetCell = result.gameObject.GetComponent<FoodItemCell>();
        ...
        if (targetNpc != null)
        {
            // Dropped on NPC - animate and handle
            _dragRectTransform.DOMove(targetNpc.transform.position, 0.2f)
                .SetEase(Ease.InOutQuad)
                .OnComplete(() =>
                {
                    _dragIcon.gameObject.SetActive(false);

                    // Здесь вызовете свою логику для NPC
